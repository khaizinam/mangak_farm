...
function defaultState() {
  ...
  return {
    ...
    producerSlots: [
      { inputKey: null, qtyLeft: 0, qtyReady: 0, lastUpdate: now, timeSpent: 0 },
      { inputKey: null, qtyLeft: 0, qtyReady: 0, lastUpdate: now, timeSpent: 0 },
      { inputKey: null, qtyLeft: 0, qtyReady: 0, lastUpdate: now, timeSpent: 0 }
    ]
  };
...
function loadState() {
  try {
    ...
    // Khởi tạo nhà sản xuất nếu chưa có
    if (typeof s.producerUnlocked === 'undefined') {
      s.producerUnlocked = false;
    }
    if (!s.producerSlots || s.producerSlots.length !== 3) {
      s.producerSlots = [
        { inputKey: null, qtyLeft: 0, qtyReady: 0, lastUpdate: Date.now(), timeSpent: 0 },
        { inputKey: null, qtyLeft: 0, qtyReady: 0, lastUpdate: Date.now(), timeSpent: 0 },
        { inputKey: null, qtyLeft: 0, qtyReady: 0, lastUpdate: Date.now(), timeSpent: 0 }
      ];
    }
...
function lazyUpdateAll() {
  ...
  const producerChanged = updateProducer(now);
  if (producerChanged) changed = true;
...
function sellItem(inventoryKey, qty = 1) {
  ...
  let revenue = 0;
  if (inventoryKey === 'cheese') {
    revenue = 2500 * qty;
  } else if (inventoryKey === 'harvest_chicken_egg') {
    revenue = 50 * qty;
...
// ============================================================
// MATERIAL PRODUCER ACTIONS
// ============================================================
function getProducerOutputInfo(inputKey) {
  if (!inputKey) return null;
  if (inputKey === 'harvest_cow_milk') {
    return {
      key: 'cheese',
      name: 'Phô mai',
      emoji: '🧀',
  ...
  } else if (inputKey.startsWith('harvest_')) {
    const plantId = inputKey.replace('harvest_', '');
    const plant = PLANTS_DATA[plantId];
    if (plant) {
      return {
        key: plantId,
...
function unlockProducer() {
  if (G.producerUnlocked) return { ok: false, msg: 'Nhà sản xuất vật tư đã được mở khoá từ trước!' };
  if (G.gold < 100000) return { ok: false, msg: 'Không đủ vàng để mở khoá (Cần 100.000 🪙)!' };
  G.gold -= 100000;
  G.producerUnlocked = true;
  saveState();
  return { ok: true };
}

function loadProducerSlot(slotIndex, inputKey, qty) {
  if (!G.producerUnlocked) return { ok: false, msg: 'Nhà sản xuất vật tư chưa được mở khoá!' };
  const slot = G.producerSlots[slotIndex];
  if (!slot) return { ok: false, msg: 'Slot không hợp lệ!' };
  if (slot.inputKey && slot.inputKey !== inputKey) {
    return { ok: false, msg: 'Slot đang sản xuất vật phẩm khác, không thể trộn lẫn!' };
  }
  
  const outInfo = getProducerOutputInfo(inputKey);
  const ratio = outInfo ? (outInfo.inputRatio || 1) : 1;
  
  if (inputKey === 'harvest_cow_milk' && qty % ratio !== 0) {
    return { ok: false, msg: `Số lượng sữa bò nạp vào phải là bội số của ${ratio}!` };
  }
  
  const currentInputEquiv = slot.qtyLeft + slot.qtyReady * ratio;
  if (currentInputEquiv + qty > 64) {
    return { ok: false, msg: `Vượt quá giới hạn chứa tối đa 64 nguyên liệu trong một slot (Hiện có tương đương: ${currentInputEquiv})!` };
  }
  
  const hasQty = G.inventory[inputKey] || 0;
  if (hasQty < qty) return { ok: false, msg: 'Không đủ vật phẩm trong túi đồ!' };
  
  // Khấu trừ túi đồ
  G.inventory[inputKey] -= qty;
  if (G.inventory[inputKey] <= 0) {
    delete G.inventory[inputKey];
  }
...
function collectProducerSlot(slotIndex) {
  if (!G.producerUnlocked) return { ok: false, msg: 'Nhà sản xuất vật tư chưa được mở khoá!' };
  const slot = G.producerSlots[slotIndex];
  if (!slot || slot.qtyReady <= 0) return { ok: false, msg: 'Không có vật phẩm nào đã hoàn thành để thu hoạch!' };
  
  const outInfo = getProducerOutputInfo(slot.inputKey);
  if (!outInfo) return { ok: false, msg: 'Vật phẩm không hợp lệ!' };
  
  const readyQty = slot.qtyReady;
  
  // Cộng vào túi đồ
  G.inventory[outInfo.key] = (G.inventory[outInfo.key] || 0) + readyQty;
  slot.qtyReady = 0;
  
  // Nếu đã hết nguyên liệu nạp và thành phẩm, dọn dẹp slot
  if (slot.qtyLeft === 0) {
    slot.inputKey = null;
    slot.timeSpent = 0;
  }
  ...
  saveState();
  return { ok: true, key: outInfo.key, name: outInfo.name, emoji: outInfo.emoji, qty: readyQty };
}

function updateProducer(now = Date.now()) {
  let changed = false;
  if (!G.producerUnlocked || !G.producerSlots) return false;
  
  G.producerSlots.forEach(slot => {
    if (slot.inputKey && slot.qtyLeft > 0) {
      if (!slot.lastUpdate) slot.lastUpdate = now;
      const elapsed = now - slot.lastUpdate;
      slot.timeSpent += elapsed;
      slot.lastUpdate = now;
      
      const outInfo = getProducerOutputInfo(slot.inputKey);
      const singleItemTime = outInfo ? outInfo.time : 5 * 60 * 1000;
      const inputRatio = outInfo ? (outInfo.inputRatio || 1) : 1;
      
      if (slot.timeSpent >= singleItemTime) {
        const produced = Math.floor(slot.timeSpent / singleItemTime);
        const maxPossible = Math.floor(slot.qtyLeft / inputRatio);
        const actual = Math.min(produced, maxPossible);
        
        if (actual > 0) {
          slot.qtyReady += actual;
          slot.qtyLeft -= actual * inputRatio;
          slot.timeSpent = slot.timeSpent % singleItemTime;
          if (slot.qtyLeft < inputRatio) {
            slot.timeSpent = 0;
          }
          changed = true;
        }
      }
    } else if (slot.inputKey && slot.qtyLeft === 0) {
      slot.lastUpdate = now;
    }
  });
...
window.GAME = {
  ...
  unlockProducer, loadProducerSlot, collectProducerSlot, updateProducer, getProducerOutputInfo,
  updateSeasonClock, getDayOfSeason, resetGame, setStartSeason
};

// Expose everything in GAME directly to window for global access in submodules
for (const key in window.GAME) {
  window[key] = window.GAME[key];
}
